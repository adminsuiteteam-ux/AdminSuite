...
export function FinancialChart({ formatValue }) {
  const colors = useColors();
  const [range, setRange] = useState("7D");
  const [size, setSize] = useState({ w: 0, h: 240 });

  const data = useMemo(() => {
    const r = chartRanges[range];
    const profit = r.income.map((v, i) => v - r.expense[i]);
    return { ...r, profit };
  }, [range]);
  ...
  const buildPoints = (arr) => arr.map((v, i) => ({ x: xFor(i), y: yFor(v) }));
...
function SummaryPill({ color, label, value, large }) {
  return (
